...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
    ...
    <div class="h-screen flex flex-col items-center justify-center gap-8">
        <div class="relative">
            ...
            <div class="absolute -bottom-8 left-1/2 -translate-x-1/2 w-8 h-2 bg-black/20 dark:bg-white/20 rounded-full bee-shadow"></div>
        </div>
        <p class="text-transparent bg-clip-text bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-500 text-lg font-medium animate-pulse tracking-wider">
            Buzzing into action...
        </p>
    </div>
    {:else if $auth}
    <div class="flex" in:fade={{ duration: 200 }}>
        ...
        <main class="flex-1 p-8 dark:text-gray-100 relative">
            {#if $navigating}
            <div class="absolute inset-0 bg-gray-50/50 dark:bg-gray-900/50 flex flex-col items-center justify-center gap-4 z-50">
                <div class="relative">
                    <div class="text-4xl bee-flight">🐝</div>
                    <div class="absolute -bottom-6 left-1/2 -translate-x-1/2 w-6 h-1.5 bg-black/20 dark:bg-white/20 rounded-full bee-shadow"></div>
                </div>
            </div>
            {/if}
            <slot />
